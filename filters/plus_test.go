...
func TestPlusAnIntToAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := PlusFactory(nil, []core.Value{intValue(5)})
	spec.Expect(filter(43, nil).(int)).ToEqual(48)
}

func TestPlusAFloattToAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := PlusFactory(nil, []core.Value{floatValue(5.11)})
...
func TestPlusAnIntToNow(t *testing.T) {
	spec := gspec.New(t)
	filter := PlusFactory(nil, []core.Value{intValue(61)})
...
func TestPlusAnIntToATime(t *testing.T) {
	spec := gspec.New(t)
	now := time.Now()
	filter := PlusFactory(nil, []core.Value{intValue(7)})
...
func TestPlusAnIntToAStringAsAFloat(t *testing.T) {
	spec := gspec.New(t)
	filter := PlusFactory(nil, []core.Value{floatValue(2.2)})
...
func TestPlusAnDynamicIntToBytesAsAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := PlusFactory(nil, []core.Value{dynamicValue("fee")})
