...
func TestMinusAnIntToAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := MinusFactory(nil, []core.Value{intValue(5)})
	spec.Expect(filter(43, nil).(int)).ToEqual(38)
}

func TestMinusAFloattToAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := MinusFactory(nil, []core.Value{floatValue(5.11)})
...
func TestMinusAnIntToATime(t *testing.T) {
	spec := gspec.New(t)
	now := time.Now()
	filter := MinusFactory(nil, []core.Value{intValue(7)})
...
func TestMinusAnIntToAStringAsAFloat(t *testing.T) {
	spec := gspec.New(t)
	filter := MinusFactory(nil, []core.Value{floatValue(2.2)})
...
func TestMinusAnDynamicIntToBytesAsAnInt(t *testing.T) {
	spec := gspec.New(t)
	filter := MinusFactory(nil, []core.Value{dynamicValue("fee")})
