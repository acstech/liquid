...
func TestTruncateWordsWhenTooLong(t *testing.T) {
	spec := gspec.New(t)
	filter := TruncateWordsFactory(nil, []core.Value{intValue(2)})
...
func TestTruncateWordsWhenTooShort(t *testing.T) {
	spec := gspec.New(t)
	filter := TruncateWordsFactory(nil, []core.Value{intValue(6)})
...
func TestTruncateWordsWhenJustRight(t *testing.T) {
	spec := gspec.New(t)
	filter := TruncateWordsFactory(nil, []core.Value{intValue(5)})
	spec.Expect(filter("hello world how's it going", nil).(string)).ToEqual("hello world how's it going")
}

func TestTruncateWordsWithCustomAppend(t *testing.T) {
	spec := gspec.New(t)
	filter := TruncateWordsFactory(nil, []core.Value{intValue(3), stringValue(" (more...)")})
...
func TestTruncateWordsWithShortWords(t *testing.T) {
	spec := gspec.New(t)
	filter := TruncateWordsFactory(nil, []core.Value{dynamicValue("max")})
