...
func TestOutputHandlesEmptyOutput(t *testing.T) {
	spec := gspec.New(t)
	output, err := newOutput(core.NewParser(nil, []byte("{{}}")))
...
func TestOutputWithASingleFilter(t *testing.T) {
	output, _ := newOutput(core.NewParser(nil, []byte("{{'fun' | debug }}")))
	assertFilters(t, output, "debug(0)")
}

func TestOutputWithMultipleFilters(t *testing.T) {
	output, _ := newOutput(core.NewParser(nil, []byte("{{'fun' | debug | debug}}")))
	assertFilters(t, output, "debug(0)", "debug(1)")
}

func TestOutputWithMultipleFiltersHavingParameters(t *testing.T) {
	spec := gspec.New(t)
	output, err := newOutput(core.NewParser(nil, []byte("{{'fun' | debug:1,2 | debug:'test' | debug : 'test' , 5}}")))
	spec.Expect(err).ToBeNil()
...
func TestOutputWithAnEscapeParameter(t *testing.T) {
	spec := gspec.New(t)
	output, err := newOutput(core.NewParser(nil, []byte("{{'fun' | debug: 'te\\'st'}}")))
	spec.Expect(err).ToBeNil()
